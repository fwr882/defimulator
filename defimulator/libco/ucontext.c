...
* 2 - to demonstrate the power and speed of libco over existing
*     implementations, such as pth (which defaults to wrapping ucontext
*     on UNIX targets).
*
...
cothread_t co_create(unsigned int heapsize, void (*coentry)(void))
{
    ...
    if (thread) {
        if ((!getcontext(thread) && !(thread->uc_stack.ss_sp = 0)) &&
            (thread->uc_stack.ss_sp = malloc(heapsize))) {
