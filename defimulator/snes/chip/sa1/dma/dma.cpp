...
void SA1::dma_normal(void)
{
    while (mmio.dtc--) {
        ...
        if (mmio.sd == DMA::SourceBWRAM && mmio.dd == DMA::DestBWRAM) {
            continue;
        }

        if (mmio.sd == DMA::SourceIRAM  && mmio.dd == DMA::DestIRAM ) {
            continue;
        }
        ...
        if (mmio.dd == DMA::DestBWRAM) {
            if ((dda & 0x40e000) == 0x006000 ||
                (dda & 0xf00000) == 0x400000) {
    ...
    if ((addr & charmask) == 0) {
        ...
        for (unsigned y = 0; y < 8; y++) {
            ...
            for (unsigned byte = 0; byte < bpp; byte++) {
                data |= (uint64)memory::cartram.read((bwaddr + byte) & bwmask)
                    << (byte << 3);
            }
            bwaddr += bpl;
