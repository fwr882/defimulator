...
void PPU::Screen::run(void)
{
    uint16 color;
    if (self.regs.pseudo_hires == false && self.regs.bgmode != 5 &&
        self.regs.bgmode != 6) {
        color = get_pixel(0);
        *output++ = color;
        *output++ = color;
...
uint16 PPU::Screen::get_pixel(bool swap)
{
    ...
    bool color_enable[] = {
        regs.bg1_color_enable,
        regs.bg2_color_enable,
        regs.bg3_color_enable,
        regs.bg4_color_enable,
        regs.oam_color_enable,
        regs.back_color_enable
    };
    ...
    if (self.bg1.output.main.priority) {
        ...
        if (regs.direct_color && (self.regs.bgmode == 3 ||
            self.regs.bgmode == 4 || self.regs.bgmode == 7)) {
            color_main = get_direct_color(self.bg1.output.main.palette,
                self.bg1.output.main.tile);
        } else {
    ...
    if (priority_sub == 0) {
        if (self.regs.pseudo_hires == true || self.regs.bgmode == 5 ||
            self.regs.bgmode == 6) {
            color_sub = get_color(0);
        } else {
            color_sub = (regs.color_b << 10) + (regs.color_g << 5) +
                (regs.color_r << 0);
        }
        source_sub = BACK;
    }
    ...
    if (!color_exempt && color_enable[source_main] &&
        self.window.output.sub.color_enable) {
        bool halve = false;
