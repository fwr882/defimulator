...
void CPU::synchronize_smp(void)
{
    if (SMP::Threaded == true) {
        if (smp.clock < 0) {
            co_switch(smp.thread);
        }
    } else {
        while (smp.clock < 0) {
            smp.enter();
...
void CPU::synchronize_ppu(void)
{
    if (PPU::Threaded == true) {
        if (ppu.clock < 0) {
            co_switch(ppu.thread);
        }
    } else {
        while (ppu.clock < 0) {
            ppu.enter();
...
void CPU::synchronize_coprocessor(void)
{
    for (unsigned i = 0; i < coprocessors.size(); i++) {
        Processor &chip = *coprocessors[i];
        if (chip.clock < 0) {
            co_switch(chip.thread);
        }
    }
}
...
alwaysinline void CPU::op_step(void)
{
...
CPU::CPU(void) : queue(512, { &CPU::queue_event, this })
{
