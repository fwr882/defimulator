...
bool CPU::dma_transfer_valid(uint8 bbus, unsigned abus)
{
  /* transfers from WRAM to WRAM are invalid; chip only has one address bus */
    if (bbus == 0x80 && ((abus & 0xfe0000) == 0x7e0000 ||
        (abus & 0x40e000) == 0x0000)) {
        return false;
    }
...
uint8 CPU::dma_bbus(unsigned i, unsigned index)
{
    ...
    case 1:
        /* 0,1 */
        return (channel[i].dest_addr + (index & 1));
    case 2:
        /* 0,0 */
        return (channel[i].dest_addr);
    case 3:
        /* 0,0,1,1 */
        return (channel[i].dest_addr + ((index >> 1) & 1));
    case 4:
        /* 0,1,2,3 */
        return (channel[i].dest_addr + (index & 3));
    case 5:
        /* 0,1,0,1 */
        return (channel[i].dest_addr + (index & 1));
    case 6:
...
{
    unsigned result =
        (channel[i].source_bank << 16) | (channel[i].source_addr);
...
void CPU::hdma_update(unsigned i)
{
    if ((channel[i].line_counter & 0x7f) == 0) {
        ...
        /*
        * Emulating this glitch causes a slight slowdown;
...
void CPU::hdma_run(void)
{
    ...
    for (unsigned i = 0; i < 8; i++) {
        ...
        if (channel[i].hdma_do_transfer) {
            static const unsigned transfer_length[] = {
                1, 2, 2, 4, 4, 4, 2, 4 };
            unsigned length = transfer_length[channel[i].transfer_mode];
            for (unsigned index = 0; index < length; index++) {
                unsigned addr = 0;
                if (channel[i].indirect == false) {
                    addr = hdma_addr(i);
                } else {
                    addr = hdma_iaddr(i);
                }
