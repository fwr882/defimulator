...
uint8 CPU::mmio_read(unsigned addr)
{
    ...
    /* TODO: Fix this switch statement to remove curly braces per case */
    switch(addr & 0xffff) {
    case 0x2180: {
    ...
    case 0x4212: {
        ...
        if (vcounter() >= vbstart && vcounter() <= vbstart + 2) {
            result |= 0x01;
        }
...
void CPU::mmio_write(unsigned addr, uint8 data)
{
    ...
    /* TODO: Fix switch statement to remove curly braces from cases. */
    switch(addr & 0xffff) {
    case 0x2180: {
    ...
    case 0x4206: {
        ...
        status.rddiv = 0xffff;
        if (status.wrdivb) {
            status.rddiv = status.wrdiva / status.wrdivb;
        }

        status.rdmpy = status.wrdiva;
        if (status.wrdivb) {
            status.wrdiva % status.wrdivb;
        }
    ...
    if ((addr & 0xff80) == 0x4300) {
        ...
        case 0x4302: {
            channel[i].source_addr = (channel[i].source_addr & 0xff00) |
                (data << 0);
            return;
        }
        case 0x4303: {
            channel[i].source_addr = (data << 8) |
                (channel[i].source_addr & 0x00ff);
            return;
        }
        case 0x4304: {
        ...
        case 0x4305: {
            channel[i].transfer_size = (channel[i].transfer_size & 0xff00) |
                (data << 0);
            return;
        }
        case 0x4306: {
            channel[i].transfer_size = (data << 8) |
                (channel[i].transfer_size & 0x00ff);
            return;
        }
        case 0x4307: {
        ...
        case 0x4308: {
            channel[i].hdma_addr = (channel[i].hdma_addr & 0xff00) |
                (data << 0);
            return;
        }
        case 0x4309: {
            channel[i].hdma_addr = (data << 8) |
                (channel[i].hdma_addr & 0x00ff);
            return;
        }
        case 0x430a: {
