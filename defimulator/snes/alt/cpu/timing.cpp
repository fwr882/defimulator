...
void CPU::add_clocks(unsigned clocks)
{
    if (status.hirq_enabled) {
        if (status.virq_enabled) {
            ...
            status.irq_valid = (cpu_time <= irq_time) &&
                (cpu_time + clocks > irq_time);
...
void CPU::scanline(void)
{
    ...
    if (status.auto_joypad_poll_enabled &&
        vcounter() == (ppu.overscan() == false ? 227 : 242)) {
        input.poll();
        run_auto_joypad_poll();
    }
}

void CPU::run_auto_joypad_poll(void)
{
