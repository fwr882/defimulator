...
uint16 PPU::get_vram_address(void)
{
...
/*
* NOTE:
* All VRAM writes during active display are invalid.  Unlike OAM and CGRAM,
* they will not be written anywhere at all.  The below address ranges for
* where writes are invalid have been validated on hardware, as has the edge
* case where the S-CPU MDR can be written if the write occurs during the very
* last clock cycle of the vblank.
*/
...
uint8 PPU::cgram_mmio_read(uint16 addr)
{
    ...
    /* XXX: Is this condition always true? */
