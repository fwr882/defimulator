...
uint8 PPUDebugger::vram_mmio_read(uint16 addr)
{
    ...
    debugger.breakpoint_test(Debugger::Breakpoint::Source::VRAM,
        Debugger::Breakpoint::Mode::Read, addr, data);
    return data;
}
...
void PPUDebugger::vram_mmio_write(uint16 addr, uint8 data)
{
    ...
    debugger.breakpoint_test(Debugger::Breakpoint::Source::VRAM,
        Debugger::Breakpoint::Mode::Write, addr, data);
}

uint8 PPUDebugger::oam_mmio_read(uint16 addr)
{
    uint8 data = PPU::oam_mmio_read(addr);
    debugger.breakpoint_test(Debugger::Breakpoint::Source::OAM,
        Debugger::Breakpoint::Mode::Read, addr, data);
    return data;
}
...
uint8 PPUDebugger::cgram_mmio_read(uint16 addr)
{
    ...
    debugger.breakpoint_test(Debugger::Breakpoint::Source::CGRAM,
        Debugger::Breakpoint::Mode::Read, addr, data);
    return data;
}
...
bool PPUDebugger::property(unsigned id, string &name, string &value)
{
    ...
    static char color_window_mask_mode[4][32] = { "Always", "Never",
        "Inside Window Only", "Outside Window Only" };

    /* $2130 */
    item("$2130", "");
    item("Color Mainscreen Window Mask",
        color_window_mask_mode[regs.color_mask]);
    item("Color Subscreen Window Mask",
        color_window_mask_mode[regs.colorsub_mask]);
    item("Color Add/Subtract Mode",
        !regs.addsub_mode ? "Fixed Color" : "Subscreen");
