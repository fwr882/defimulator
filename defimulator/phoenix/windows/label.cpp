void Label::create(Window &parent, unsigned x, unsigned y, unsigned width,
...
LRESULT CALLBACK Label_windowProc(HWND hwnd, UINT msg, WPARAM wparam,
    LPARAM lparam)
{
    Window *window_ptr = (Window*)GetWindowLongPtr(GetParent(hwnd),
        GWLP_USERDATA);
    if (!window_ptr) {
    ...
    case WM_PAINT: {
        ...
        FillRect(ps.hdc, &rc, window.window->brush ?
            window.window->brush : GetSysColorBrush(COLOR_3DFACE));
        SetBkColor(ps.hdc, window.window->brush ?
            window.window->brushColor : GetSysColor(COLOR_3DFACE));
