    ...
    unsigned width, unsigned height)
{
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(hexEditor->container),
        GTK_POLICY_NEVER, GTK_POLICY_NEVER);
    gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(
        hexEditor->container), GTK_SHADOW_ETCHED_IN);

    hexEditor->widget = gtk_text_view_new();
    gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(hexEditor->widget),
        GTK_WRAP_NONE);
    gtk_container_add(GTK_CONTAINER(hexEditor->container), hexEditor->widget);
    g_signal_connect(G_OBJECT(hexEditor->widget), "key-press-event",
        G_CALLBACK(HexEditor_keyPress), (gpointer)this);
    ...
    g_signal_connect(G_OBJECT(hexEditor->scroll), "change-value",
        G_CALLBACK(HexEditor_scroll), (gpointer)this);

    gtk_box_pack_start(GTK_BOX(object->widget), hexEditor->container,
        true, true, 0);
    gtk_box_pack_start(GTK_BOX(object->widget), hexEditor->scroll,
        false, false, 1);

    object->textBuffer = gtk_text_view_get_buffer(
        GTK_TEXT_VIEW(hexEditor->widget));
    ...
    for (unsigned row = 0; row < hexEditor->rows; row++) {
        ...
        for (unsigned column = 0; column < hexEditor->columns; column++) {
            if (offset < hexEditor->size) {
                ...
                char buffer[2] = {
                    data >= 0x20 && data <= 0x7e ? (char)data : '.', 0
                };
        ...
        if(row != hexEditor->rows - 1) {
            output.append("\n");
        }
    }

    gtk_text_buffer_set_text(object->textBuffer, output, -1);
    /* Start at first position where hex values can be entered. */
    if (position == 0) {
        position = 10;
    }
...
bool HexEditor::keyPress(unsigned scancode)
{
    ...
    unsigned position = cursorPosition();
    unsigned lineWidth = 10 + (hexEditor->columns * 3) +
        1 + (hexEditor->columns) + 1;
    ...
    if (scancode == GDK_Down) {
        if (cursorY != hexEditor->rows - 1) {
            return false;
        }

        signed newOffset = hexEditor->offset + hexEditor->columns;
        if (newOffset + hexEditor->columns * hexEditor->rows -
            (hexEditor->columns - 1) <= hexEditor->size) {
            setOffset(newOffset);
        update();
        }
        return true;
    ...
    if (cursorX >= 10) {
        ...
        cursorX -= 10;
        if ((cursorX % 3) != 2) {
            /* Not on a space; 0 = high, 1 = low. */
            bool cursorNibble = (cursorX % 3) == 1;
            cursorX /= 3;
            if (cursorX < hexEditor->columns) {
                /* Not in ANSI region. */
                unsigned offset = hexEditor->offset +
                    (cursorY * hexEditor->columns + cursorX);

                /* Do not edit past the end of the file. */
                if (offset >= hexEditor->size) {
                    return false;  //do not edit past end of file
                }
                ...
                position++;
                if (cursorNibble && cursorX != hexEditor->columns - 1) {
                    position++;
                }
...
{
    GtkTextIter iter;
    gtk_text_buffer_get_iter_at_mark(object->textBuffer,
        &iter, hexEditor->cursor);
...
{
    GtkTextIter iter;
    gtk_text_buffer_get_iter_at_mark(object->textBuffer, &iter,
        hexEditor->cursor);

    /*
    * GTK+ will throw a hundred errors on the terminal if you
    * set an iterator past the end of the text buffer.
    */
    GtkTextIter endIter;
