...
{
    signed selection = -1;
    if(auto position = self->selection()) {
        selection = position();
    }
...
static void ListBox_change(ListBox *self)
{
    ...
    if (selection == self->listBox->selection) {
        return;
    }
    ...
    unsigned height, const string &text)
{
    ...
    object->subWidget = gtk_tree_view_new_with_model(
        GTK_TREE_MODEL(listBox->store));
    ...
    for (unsigned i = 0; i < list.size(); i++) {
        if (i == 0) {
            ...
            listBox->column[i].column =
                gtk_tree_view_column_new_with_attributes("",
                    listBox->column[i].renderer, "active", i, (void*)0);
            gtk_tree_view_column_set_resizable(listBox->column[i].column,
                false);
            gtk_tree_view_column_set_visible(listBox->column[i].column,
                listBox->checkable);
            g_signal_connect(listBox->column[i].renderer, "toggled",
                G_CALLBACK(ListBox_tick), (gpointer)this);
        } else {
        ...
        gtk_tree_view_column_set_widget(GTK_TREE_VIEW_COLUMN(
            listBox->column[i].column), listBox->column[i].label);
        gtk_tree_view_append_column(GTK_TREE_VIEW(object->subWidget),
        listBox->column[i].column);
    ...
    // >= 2 + one for the checkbox column
    gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(object->subWidget),
        list.size() >= 3);

    gtk_tree_view_set_search_column(GTK_TREE_VIEW(object->subWidget), 1);

    g_signal_connect_swapped(G_OBJECT(object->subWidget), "cursor-changed",
        G_CALLBACK(ListBox_change), (gpointer)this);
    g_signal_connect_swapped(G_OBJECT(object->subWidget), "row-activated",
        G_CALLBACK(ListBox_activate), (gpointer)this);
...
{
    gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(object->subWidget),
        visible);
}
...
void ListBox::setCheckable(bool checkable)
{
    ...
    if (object->subWidget) {
        gtk_tree_view_column_set_visible(listBox->column[0].column,
            checkable);
    }
}
...
void ListBox::setFont(Font &font)
{
    ...
    /* XXX: Potential replacement:
    *  while (gtk_tree_view_get_column(...) != 0) {
    *      columns++;
    *  }
    */
    while (true) {
        if (gtk_tree_view_get_column(GTK_TREE_VIEW(object->subWidget),
            columns) == 0) {
            break;
        }
        columns++;
...
void ListBox::reset(void)
{
    ...
    gtk_tree_view_set_model(GTK_TREE_VIEW(object->subWidget),
        GTK_TREE_MODEL(listBox->store));
    /*
    * Reset gtk_scrolled_window scrollbar position to (0,0) -- top-left -- as
    * ListBox is now empty.
    */
    gtk_scrolled_window_set_hadjustment(
        GTK_SCROLLED_WINDOW(object->widget), 0);
...
void ListBox::setItem(unsigned row, const string &text)
{
    ...
    model = gtk_tree_view_get_model(GTK_TREE_VIEW(object->subWidget));
...
bool ListBox::checked(unsigned row)
{
    ...
    model = gtk_tree_view_get_model(GTK_TREE_VIEW(object->subWidget));
    path = gtk_tree_path_new_from_string(string(row));
...
optional<unsigned> ListBox::selection(void)
{
    ...
    selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(object->subWidget));
    model = gtk_tree_view_get_model(GTK_TREE_VIEW(object->subWidget));
...
void ListBox::setSelection(unsigned row)
{
    ...
    /* XXX:
    * This variable has the same name as the function.  See above.  If you
    * move this variable above the if-statement, the compiler will complain
    * about trying to use the variable as a function.  Bad form, I think.
    */
    GtkTreeSelection *selection;
    ...
    /* TODO: Since there's no condition, a while loop should do fine:
    * unsigned i = 1;
    * while (true) {
