...
struct SNESSMP {
    ...
    static string disassemble(uint16_t pc, uint8_t opcode, uint8_t pl,
        uint8_t ph);
    static string disassemble(uint16_t pc, bool p, uint8_t opcode,
        uint8_t pl, uint8_t ph);
};
...
inline string SNESSMP::disassemble(uint16_t pc, uint8_t opcode, uint8_t pl,
    uint8_t ph)
{
    ...
    /* XXX: Would a switch statement be a better choice? */
    ...
    if (mode == TVector) {
        return { name, " ", opcode >> 4 };
    }

    if (mode == Direct) {
        return { name, " $", hex<2>(pl) };
    }

    if (mode == DirectRelative) {
        return { name, " $", hex<2>(pl), ",$", hex<4>(pc + 3 + (int8_t)ph) };
    }

    if (mode == ADirect) {
        return { name, " a,$", hex<2>(pl) };
    }

    if (mode == AAbsolute) {
        return { name, " a,$", hex<4>(pa) };
    }
    ...
    if (mode == DirectDirect) {
        return { name, " $", hex<2>(ph), ",$", hex<2>(pl) };
    }

    if (mode == CAbsoluteBit) {
        return { name, " c,$", hex<4>(pa & 0x1fff), ":", pa >> 13 };
    }

    if (mode == Absolute) {
        return { name, " $", hex<4>(pa) };
    }
    ...
    if (mode == AbsoluteA) {
        return { name, " $", hex<4>(pa), ",a" };
    }

    if (mode == Relative) {
        return { name, " $", hex<4>(pc + 2 + (int8_t)pl) };
    }
    ...
    if (mode == AAbsoluteX) {
        return { name, " a,$", hex<4>(pa), "+x" };
    }

    if (mode == AAbsoluteY) {
        return { name, " a,$", hex<4>(pa), "+y" };
    }

    if (mode == AIDirectY) {
        return { name, " a,($", hex<2>(pl), ")+y" };
    }

    if (mode == DirectConstant) {
        return { name, " $", hex<2>(ph), ",#$", hex<2>(pl) };
    }
    ...
    if (mode == XAbsolute) {
        return { name, " x,$", hex<4>(pa) };
    }

    if (mode == IAbsoluteX) {
        return { name, " ($", hex<4>(pa), "+x)" };
    }

    if (mode == CNAbsoluteBit) {
        return { name, " c,!$", hex<4>(pa & 0x1fff), ":", pa >> 13 };
    }

    if (mode == XDirect) {
        return { name, " x,$", hex<2>(pl) };
    }

    if (mode == PVector) {
        return { name, " $ff", hex<2>(pl) };
    }

    if (mode == YaDirect) {
        return { name, " ya,$", hex<2>(pl) };
    }
    ...
    if (mode == AbsoluteBitC) {
        return { name, " $", hex<4>(pa & 0x1fff), ":", pa >> 13, ",c" };
    }

    if (mode == DirectY) {
        return { name, " $", hex<2>(pl), ",y" };
    }

    if (mode == AbsoluteY) {
        return { name, " $", hex<4>(pa), ",y" };
    }
    ...
    if (mode == AbsoluteXA) {
        return { name, " $", hex<4>(pa), "+x,a" };
    }

    if (mode == AbsoluteYA) {
        return { name, " $", hex<4>(pa), "+y,a" };
    }

    if (mode == IDirectYA) {
        return { name, " ($", hex<2>(pl), ")+y,a" };
    }

    if (mode == DirectYX) {
        return { name, " $", hex<2>(pl), "+y,x" };
    }

    if (mode == DirectYa) {
        return { name, " $", hex<2>(pl), ",ya" };
    }

    if (mode == DirectXY) {
        return { name, " $", hex<2>(pl), "+x,y" };
    }
    ...
    if (mode == DirectXRelative) {
        return { name, " $", hex<2>(pl), ",$", hex<4>(pc + 3 + (int8_t)ph) };
    }

    if (mode == XDirectY) {
        return { name, " x,$", hex<2>(pl), "+y" };
    }

    if (mode == YDirectX) {
        return { name, " y,$", hex<2>(pl), "+x" };
    }
    ...
    if (mode == YRelative) {
        return { name, " y,$", hex<4>(pc + 2 + (int8_t)pl) };
    }

    return "";
}

inline string SNESSMP::disassemble(uint16_t pc, bool p, uint8_t opcode,
    uint8_t pl, uint8_t ph)
{
    string name = opcodeInfo[opcode].name;
    unsigned mode = opcodeInfo[opcode].mode;
    ...
    if (mode == Direct) {
        return { name, " $", hex<3>(pdl) };
    }

    if (mode == DirectRelative) {
        return { name, " $", hex<3>(pdl), ",$", hex<4>(pc + 3 + (int8_t)ph) };
    }

    if (mode == ADirect) {
        return { name, " a,$", hex<3>(pdl) };
    }
    ...
    if (mode == DirectDirect) {
        return { name, " $", hex<3>(pdh), ",$", hex<3>(pdl) };
    }
    ...
    if (mode == AIDirectY) {
        return { name, " a,($", hex<3>(pdl), ")+y" };
    }

    if (mode == DirectConstant) {
        return { name, " $", hex<3>(pdh), ",#$", hex<2>(pl) };
    }
    ...
    if (mode == IDirectYA) {
        return { name, " ($", hex<3>(pdl), ")+y,a" };
    }

    if (mode == DirectYX) {
        return { name, " $", hex<3>(pdl), "+y,x" };
    }

    if (mode == DirectYa) {
        return { name, " $", hex<3>(pdl), ",ya" };
    }

    if (mode == DirectXY) {
        return { name, " $", hex<3>(pdl), "+x,y" };
    }
    ...
    if (mode == XDirectY) {
        return { name, " x,$", hex<3>(pdl), "+y" };
    }

    if (mode == YDirectX) {
        return { name, " y,$", hex<3>(pdl), "+x" };
    }
