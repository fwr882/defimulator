...
struct directory {
    ...
    static lstring files(const string &pathname,
        const string &pattern = "*");
    static lstring contents(const string &pathname,
        const string &pattern = "*");
};
...
inline lstring directory::folders(const string &pathname,
    const string &pattern)
{
    lstring list;
...
inline lstring directory::contents(const string &pathname,
    const string &pattern)
{
    ...
    const string &pattern)
{
    ...
    if (dp) {
        while (ep = readdir(dp)) {
            ...
            if (ep->d_type & DT_DIR) {
                if (wildcard(ep->d_name, pattern)) {
                    list.append(string(ep->d_name, "/"));
                }
            }
        }
