...
public:
    ...
    {
        /* round to nearest power of two (for amortized growth) */
        newsize = bit::round(newsize);

        T *poolcopy = (T*)malloc(newsize * sizeof(T));
        for (unsigned i = 0; i < min(objectsize, newsize); i++) {
            new(poolcopy + i) T(pool[i]);
        }
    ...
    void resize(unsigned newsize)
    {
        ...
        if (newsize < objectsize) {
            ...
            for (unsigned i = objectsize; i < newsize; i++) {
                new(pool + i) T;
            }
        }
    ...
    void remove(unsigned index, unsigned count = 1)
    {
        ...
        if (count + index >= objectsize) {
            resize(index);
        } else {
            resize(objectsize - count);
        }
    }
    ...
    linear_vector(const linear_vector<T> &source)
        : pool(0), poolsize(0), objectsize(0)
    {
        operator=(source);
    }
    ...
    linear_vector(std::initializer_list<T> list)
        : pool(0), poolsize(0), objectsize(0)
    {
...
public:
    unsigned size(void) const { return objectsize; }
    unsigned capacity(void) const { return poolsize; }
    ...
    void reserve(unsigned newsize)
    {
        ...
        for (unsigned i = poolsize; i < newsize; i++) {
            pool[i] = 0;
        }
        poolsize = newsize;
        objectsize = min(objectsize, newsize);
    ...
    inline const T& operator[](unsigned index) const
    {
        if (index >= objectsize || !pool[index]) {
            throw "vector[] out of bounds";
        }
    ...
    pointer_vector(std::initializer_list<T> list)
        : pool(0), poolsize(0), objectsize(0)
    {
        for (const T *p = list.begin(); p != list.end(); ++p) {
            append(*p);
...
template<typename T> struct has_size<linear_vector<T>>
    { enum { value = true }; };
template<typename T> struct has_size<pointer_vector<T>>
    { enum { value = true }; };
}
