...
* This allows construction of opcodes that can take any register as input or
* output, despite the fact that behind-the-scenes, special handling is done
* for eg: F, A, HL, etc.  Registers can also be chained together: eg
* af = 0x0000 writes to both a and f.
*/
struct Register {
    virtual operator unsigned() const = 0;
    virtual unsigned operator=(unsigned x) = 0;
    Register& operator=(const Register &x)
        { operator=((unsigned)x); return *this; }

    unsigned operator++(int)
        { unsigned r = *this; operator=(*this + 1); return r; }
    unsigned operator--(int)
        { unsigned r = *this; operator=(*this - 1); return r; }
...
struct RegisterF : Register {
    ...
    unsigned operator=(unsigned x) {
        z = x & 0x80; n = x & 0x40; h = x & 0x20;
        c = x & 0x10; return *this;
    }
...
struct Registers {
    ...
    Register& operator[](unsigned r) {
        static Register* table[] = { &a, &f, &af, &b, &c, &bc, &d,
            &e, &de, &h, &l, &hl, &sp, &pc };
        return *table[r];
    }
