...
void Application::main(int argc, char **argv)
{
    ...
    * Make this configurable! */
    ...
    if (config.video.driver == "") {
        config.video.driver = video.default_driver();
    }

    if (config.audio.driver == "") {
        config.audio.driver = audio.default_driver();
    }

    if (config.input.driver == "") {
        config.input.driver = input.default_driver();
    }
    ...
    while (quit == false) {
        ...
        if (SNES::cartridge.loaded()) {
            if (application.pause == true ||
              (config.settings.focusPolicy == 0 &&
...
void Application::addWindow(TopLevelWindow *window, const string &name,
...
void Application::loadGeometry()
{
    ...
    nall_foreach(window, windows) {
        ...
        window->setGeometry(decimal(position[0]), decimal(position[1]),
