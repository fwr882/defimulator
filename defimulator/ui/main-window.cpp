...
void MainWindow::create(void)
{
    ...
    systemLoadCartridgeBsxSlotted.create(systemLoadCartridgeSpecial,
        "Load BS-X Slotted Cartridge ...");
    systemLoadCartridgeBsx.create(systemLoadCartridgeSpecial,
        "Load BS-X Cartridge ...");
    systemLoadCartridgeSufamiTurbo.create(systemLoadCartridgeSpecial,
        "Load Sufami Turbo Cartridge ...");
    systemLoadCartridgeSuperGameBoy.create(systemLoadCartridgeSpecial,
    ...
    if (config.controller.port1 == 0) {
        systemPort1None.setChecked();
    }

    if (config.controller.port1 == 1) {
        systemPort1Gamepad.setChecked();
    }

    if (config.controller.port1 == 2) {
        systemPort1Multitap.setChecked();
    }

    if (config.controller.port1 == 3) {
        systemPort1Mouse.setChecked();
    }
    ...
    if (config.controller.port2 == 0) {
        systemPort2None.setChecked();
    }

    if (config.controller.port2 == 1) {
        systemPort2Gamepad.setChecked();
    }

    if (config.controller.port2 == 2) {
        systemPort2Multitap.setChecked();
    }

    if (config.controller.port2 == 3) {
        systemPort2Mouse.setChecked();
    }

    if (config.controller.port2 == 4) {
        systemPort2SuperScope.setChecked();
    }

    if (config.controller.port2 == 5) {
        systemPort2Justifier.setChecked();
    }

    if (config.controller.port2 == 6) {
        systemPort2Justifiers.setChecked();
    }
    ...
    if (config.video.scale == 1) {
        settingsVideoMode1x.setChecked();
    }

    if (config.video.scale == 2) {
        settingsVideoMode2x.setChecked();
    }

    if (config.video.scale == 3) {
        settingsVideoMode3x.setChecked();
    }

    if (config.video.scale == 4) {
        settingsVideoMode4x.setChecked();
    }

    if (config.video.scale == 5) {
        settingsVideoMode5x.setChecked();
    }

    settingsVideoModeSeparator1.create(settingsVideoMode);
    settingsVideoModeAspectRatioCorrection.create(settingsVideoMode,
        "Correct Aspect Ratio");
    settingsVideoModeAspectRatioCorrection.setChecked(
        config.video.aspectRatioCorrection);
    ...
    if (config.video.region == 0) {
        settingsVideoModeNTSC.setChecked();
    }

    if (config.video.region == 1) {
        settingsVideoModePAL.setChecked();
    }
    ...
    systemLoadCartridge.onTick = []() {
        fileBrowser.fileOpen(FileBrowser::Mode::Cartridge,
            [](string filename) { cartridge.loadNormal(filename); });
    };

    systemLoadCartridgeBsxSlotted.onTick = []() {
        singleSlotLoader.loadCartridgeBsxSlotted();
    };

    systemLoadCartridgeBsx.onTick = []() {
        singleSlotLoader.loadCartridgeBsx();
    };

    systemLoadCartridgeSufamiTurbo.onTick = []() {
        doubleSlotLoader.loadCartridgeSufamiTurbo();
    };

    systemLoadCartridgeSuperGameBoy.onTick = []() {
        singleSlotLoader.loadCartridgeSuperGameBoy();
    };
    ...
    systemPort1None.onTick = []() {
        config.controller.port1 = 0; utility.setControllers();
    };

    systemPort1Gamepad.onTick = []() {
        config.controller.port1 = 1; utility.setControllers();
    };

    systemPort1Multitap.onTick = []() {
        config.controller.port1 = 2; utility.setControllers();
    };

    systemPort1Mouse.onTick = []() {
        config.controller.port1 = 3; utility.setControllers();
    };

    systemPort2None.onTick = []() {
        config.controller.port2 = 0; utility.setControllers();
    };

    systemPort2Gamepad.onTick = []() {
        config.controller.port2 = 1; utility.setControllers();
    };

    systemPort2Multitap.onTick = []() {
        config.controller.port2 = 2; utility.setControllers();
    };

    systemPort2Mouse.onTick = []() {
        config.controller.port2 = 3; utility.setControllers();
    };

    systemPort2SuperScope.onTick = []() {
        config.controller.port2 = 4; utility.setControllers();
    };

    systemPort2Justifier.onTick = []() {
        config.controller.port2 = 5; utility.setControllers();
    };

    systemPort2Justifiers.onTick = []() {
        config.controller.port2 = 6; utility.setControllers();
    };
    ...
    settingsVideoModeAspectRatioCorrection.onTick = []() {
        config.video.aspectRatioCorrection =
            mainWindow.settingsVideoModeAspectRatioCorrection.checked();
        utility.setScale();
    };

    settingsVideoModeNTSC.onTick = []() {
        config.video.region = 0;
        utility.setScale();
    };

    settingsVideoModePAL.onTick = []() {
        config.video.region = 1;
        utility.setScale();
    };
    ...
    settingsSynchronizeVideo.onTick = []() {
        config.video.synchronize =
            mainWindow.settingsSynchronizeVideo.checked();
        video.set(Video::Synchronize, config.video.synchronize);
    };

    settingsSynchronizeAudio.onTick = []() {
        config.audio.synchronize =
            mainWindow.settingsSynchronizeAudio.checked();
...
void MainWindow::synchronize(void)
{
