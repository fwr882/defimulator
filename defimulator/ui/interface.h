#ifndef BSNES_UI_INTERFACE_H
#define BSNES_UI_INTERFACE_H
...
struct Filter : public library {
    ...
    function<void (uint32_t*, uint32_t*, unsigned, const uint16_t*,
        unsigned, unsigned, unsigned)> dl_render;
    void size(unsigned &width, unsigned &height);
    void render(uint32_t *output, unsigned outpitch, const uint16_t *input,
...
struct Interface : public SNES::Interface {
    ...
    void input_poll();
    int16_t input_poll(bool port, SNES::Input::Device device,
