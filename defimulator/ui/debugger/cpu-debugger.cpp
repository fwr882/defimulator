...
void CPUDebugger::create(void)
{
    ...
    stepOver.create(*this, x, y, 80, Style::ButtonHeight, "Step Over");
    y += Style::ButtonHeight;

    proceed.create(*this, x, y, 80, Style::ButtonHeight, "Proceed");
    y += Style::ButtonHeight;
...
void CPUDebugger::synchronize(void)
{
    stepInto.setEnabled(SNES::cartridge.loaded() &&
        debugger.enableDebugger.checked() &&
        breakpointEditor.runToBreakpoint.checked() == false);
    stepOver.setEnabled(stepInto.enabled() && SNES::cpu.opcode_edge);
}

void CPUDebugger::refreshDisassembly(void)
{
    ...
    for (unsigned n = 0; n < 15; n++) {
        ...
        if (offset[n] == -1) {
            ...
            string text = SNESCPU::disassemble(
                ...
                read(addr + 0), read(addr + 1), read(addr + 2),
                read(addr + 3));
...
void CPUDebugger::eventStepInto(void)
{
    refreshDisassembly();
}

void CPUDebugger::eventStepOver(void)
{
    uint8_t opcode = read(SNES::cpu.regs.pc);
    unsigned length = SNESCPU::getOpcodeLength(SNES::cpu.regs.p.m,
        SNES::cpu.regs.p.x, opcode);
