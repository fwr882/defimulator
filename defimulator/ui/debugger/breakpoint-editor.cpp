...
void BreakpointEditor::create(void)
{
    ...
    runToBreakpoint.create(*this, x, y, 295,
        Style::CheckBoxHeight, "Run to breakpoint");
    ...
    for (unsigned n = 0; n < Breakpoints; n++) {
        ...
        typeBox[n].create(*this, x + 135, y, 80, Style::EditBoxHeight,
            "Exec\nRead\nWrite");
        sourceBox[n].create(*this, x + 220, y, 80, Style::EditBoxHeight,
            "CPU\nAPU\nVRAM\nOAM\nCGRAM");
...
void BreakpointEditor::toggleBreakpoint(unsigned n)
{
    ...
    if (enabled == false) {
        ...
        /* break on any value */
        if (valueBox[n].text() == "") {
            SNES::debugger.breakpoint[n].data = -1;
        }

        SNES::debugger.breakpoint[n].mode =
            (SNES::Debugger::Breakpoint::Mode)typeBox[n].selection();
        SNES::debugger.breakpoint[n].source =
            (SNES::Debugger::Breakpoint::Source)sourceBox[n].selection();
...
void BreakpointEditor::eventBreakpoint(void)
{
