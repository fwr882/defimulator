...
void Utility::updateStatus(void)
{
    ...
    if ((currentTime - statusTime) <= 3) {
        ...
        text = "Paused";
    } else if(config.settings.focusPolicy == 0 &&
...
void Utility::setControllers(void)
{
    switch (config.controller.port1) {
    case 0:
        SNES::input.port_set_device(0, SNES::Input::Device::None);
        break;
    case 1:
    ...
    case 1:
        ...
        break;
    case 2:
        SNES::input.port_set_device(1, SNES::Input::Device::Multitap);
        break;
    case 3:
        SNES::input.port_set_device(1, SNES::Input::Device::Mouse);
        break;
    case 4:
        SNES::input.port_set_device(1, SNES::Input::Device::SuperScope);
        break;
    case 5:
        SNES::input.port_set_device(1, SNES::Input::Device::Justifier);
        break;
    case 6:
        SNES::input.port_set_device(1, SNES::Input::Device::Justifiers);
        break;
    }
}
...
void Utility::setScale(unsigned scale)
{
    ...
    if (config.video.region == 0) {
        width = 256 * scale;
        height = 224 * scale;
        if (config.video.aspectRatioCorrection) {
            width *= 54.0 / 47.0;
        }
    } else {
        width = 256 * scale;
...
void Utility::setFilter(void)
{
    if (filter.opened()) {
        filter.close();
    }

    if (config.video.filter == "") {
        return;
    }
    ...
    if (filter.open_absolute(config.video.filter)) {
        ...
        if (!filter.dl_size || !filter.dl_render) {
            filter.close();
        }
    }
}

void Utility::setShader(void)
{
    string data;
...
void Utility::cartridgeLoaded(void)
{
    SNES::system.power();
    ...
    utility.showMessage({ "Loaded ", notdir(cartridge.baseName),
        cartridge.patchApplied ? ", and applied UPS patch" : ""
    });
}

void Utility::cartridgeUnloaded(void)
{
