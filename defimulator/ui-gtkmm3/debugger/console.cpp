...
void Console::create(void)
{
    ...
    traceToConsole.create(*this, x, y, 120, Style::CheckBoxHeight,
        "Trace to console");
    y += Style::CheckBoxHeight;

    traceToConsole.setChecked(true);
    traceToFile.create(*this, x, y, 120, Style::CheckBoxHeight,
        "Trace to file");
    y += Style::CheckBoxHeight;

    traceCPU.create(*this, x, y, 120, Style::CheckBoxHeight, "Trace S-CPU");
    y += Style::CheckBoxHeight;

    traceCPU.setChecked(true);
    traceSMP.create(*this, x, y, 120, Style::CheckBoxHeight, "Trace S-SMP");
    y += Style::CheckBoxHeight;

    clearConsole.create(*this, x, 338 - Style::ButtonHeight - 5, 120,
        Style::ButtonHeight, "Clear console");
    ...
    traceToFile.onTick = []() {
        console.tracerEnable(console.traceToFile.checked());
    };
...
void Console::eventTraceCPU(void)
{
    if (traceCPU.checked() == false) {
        return;
    }

    if (traceToConsole.checked() == false && traceToFile.checked() == false) {
        return;
    }
...
void Console::eventTraceSMP(void)
{
    if (traceSMP.checked() == false) {
        return;
    }
