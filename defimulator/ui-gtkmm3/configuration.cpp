...
{
    nall::configuration::load(nall::string(path.user,
        "defimulator-phoenix.cfg"));
}

void Configuration::save(void)
{
    /* XXX: mkdir is a #define in the nall/config.hpp file. */
    mkdir(path.user, 0755);
    nall::configuration::save(nall::string(path.user,
        "defimulator-phoenix.cfg"));
}
...
void Configuration::create(void)
{
    ...
    attach(video.aspectRatioCorrection = true,
        "video.aspectRatioCorrection");
