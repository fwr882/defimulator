...
int16_t InputMapper::AnalogInput::poll(InputMapper inputMapper)
{
    //int16_t value = this->state[this->activeState][this->scancode];
    int16_t value = inputMapper.state[inputMapper.activeState][scancode];
    switch (type) {
    case AbstractInput::Type::MouseAxis:
        if(nall::input.acquired() == false) {
            return 0;
        }
        return value;
    }
...
void InputMapper::Hotkeys::create(Configuration* config,
    const char *deviceName, const char *configName)
{
    name = deviceName;
...
void InputMapper::Gamepad::create(Configuration* config,
    const char *deviceName, const char *configName)
{
    name = deviceName;
...
void InputMapper::Mouse::create(Configuration* config,
    const char *deviceName, const char *configName)
{
    name = deviceName;
    x.name = "X-axis";
...
void InputMapper::SuperScope::create(Configuration* config,
    const char *deviceName, const char *configName)
{
    name = deviceName;
    x.name = "X-axis";
...
void InputMapper::Justifier::create(Configuration* config,
    const char *deviceName, const char *configName)
{
    name = deviceName;
    x.name = "X-axis";
...
void InputMapper::create(std::shared_ptr<state_t> state,
    Configuration* config)
{
    ...
    port1.gamepad.create(config, "Gamepad", "port1.gamepad");
    port1.hotkeys.create(config, "Hotkeys", "port1.hotkeys");
    port1.multitapA.create(config, "Multitap - Port 1", "port1.multitapA");
    port1.multitapB.create(config, "Multitap - Port 2", "port1.multitapB");
    port1.multitapC.create(config, "Multitap - Port 3", "port1.multitapC");
    port1.multitapD.create(config, "Multitap - Port 4", "port1.multitapD");
    port1.mouse.create(config, "Mouse", "port1.mouse");
    ...
    port2.gamepad.create(config, "Gamepad", "port2.gamepad");
    port2.multitapA.create(config, "Multitap - Port 1", "port2.multitapA");
    port2.multitapB.create(config, "Multitap - Port 2", "port2.multitapB");
    port2.multitapC.create(config, "Multitap - Port 3", "port2.multitapC");
    port2.multitapD.create(config, "Multitap - Port 4", "port2.multitapD");
    port2.mouse.create(config, "Mouse", "port2.mouse");
    port2.superScope.create(config, "Super Scope", "port2.superScope");
    port2.justifierA.create(config, "Justifier - Port 1",
        "port2.justifierA");
    port2.justifierB.create(config, "Justifier - Port 2",
        "port2.justifierB");
...
void InputMapper::poll(Configuration* config)
{
    activeState ^= 1;
    ...
    for (unsigned i = 0; i < Scancode::Limit; i++) {
        if (state[0][i] != state[1][i]) {
            poll_hotkeys(config, i, state[activeState][i]);
...
void InputMapper::poll_hotkeys(Configuration* config, unsigned scancode,
    int16_t value)
{
    /* internal state */
    ...
    if (value) {
        ...
        if (scancode == port1.hotkeys.fastforward.scancode) {
            ...
            ruby::video.set(ruby::Video::Synchronize,
                config->video.synchronize = false);
            ruby::audio.set(ruby::Audio::Synchronize,
        ...
        /*
        * Maybe there's a way for anything to poll the window to see if
        * it's focused?  That may be the job for the main window itself.
        */
        #if 0
        /* key released */
