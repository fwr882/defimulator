...
public:
    ...
    void set_shader(const char *source)
    {
        ...
        if (source) {
            ...
            if (is_glsl) {
                if (fragment_source != "") {
                    set_fragment_shader(fragment_source);
                }

                if (vertex_source != "") {
                    set_vertex_shader(vertex_source);
                }
            }
        }
    ...
    void init(void)
    {
        ...
        shader_support = glCreateProgram && glUseProgram &&
            glCreateShader && glDeleteShader && glShaderSource &&
            glCompileShader && glAttachShader && glDetachShader &&
            glLinkProgram && glGetUniformLocation && glUniform1i &&
            glUniform2fv && glUniform4fv;
