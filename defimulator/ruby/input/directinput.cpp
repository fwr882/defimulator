...
static BOOL CALLBACK
    DI_EnumJoypadAxesCallback(const DIDEVICEOBJECTINSTANCE*, void*);
...
public:
    ...
    bool poll(int16_t *table)
    {
        ...
        if (device.keyboard) {
            uint8_t state[256];
            if (FAILED(device.keyboard->GetDeviceState(
                sizeof(state), state))) {
            ...
            key(Keyboard::Shift  ) =
                (bool)(state[0x2a] & 0x80) || (bool)(state[0x36] & 0x80);
            key(Keyboard::Control) =
                (bool)(state[0x1d] & 0x80) || (bool)(state[0x9d] & 0x80);
            key(Keyboard::Alt    ) =
                (bool)(state[0x38] & 0x80) || (bool)(state[0xb8] & 0x80);
            key(Keyboard::Super  ) =
                (bool)(state[0xdb] & 0x80) || (bool)(state[0xdc] & 0x80);
            #undef key
        }
        ...
        if (device.mouse) {
            DIMOUSESTATE2 state;
            if (FAILED(device.mouse->GetDeviceState(
                sizeof(DIMOUSESTATE2), (void*)&state))) {
        ...
        /* POV hats */
            for(unsigned n = 0; n < min((unsigned)Joypad::Hats, 4); n++) {
                ...
                /*
                * Some drivers will report a centered POV hat as -1U, others
                * as 65535U.  >= 36000 will match both, as well as invalid
                * ranges.
                */
                if (pov < 36000) {
                    if (pov >= 31500 || pov <=  4500) {
                        table[joypad(i).hat(n)] |= Joypad::HatUp;
                    }

                    if (pov >=  4500 && pov <= 13500) {
                        table[joypad(i).hat(n)] |= Joypad::HatRight;
                    }

                    if (pov >= 13500 && pov <= 22500) {
                        table[joypad(i).hat(n)] |= Joypad::HatDown;
                    }

                    if (pov >= 22500 && pov <= 31500) {
                        table[joypad(i).hat(n)] |= Joypad::HatLeft;
                    }
                }
            }
            ...
            /* buttons */
            for (unsigned n = 0;
                n < min((unsigned)Joypad::Buttons, 128); n++) {
    ...
    bool init_joypad(const DIDEVICEINSTANCE *instance)
    {
        ...
        if (FAILED(device.context->CreateDevice(instance->guidInstance,
            &device.gamepad[n], 0))) {
            /* continue and try next gamepad */
            return DIENUM_CONTINUE;
        }

        device.gamepad[n]->SetDataFormat(&c_dfDIJoystick2);
        device.gamepad[n]->SetCooperativeLevel(settings.handle,
            DISCL_NONEXCLUSIVE | DISCL_BACKGROUND);
        device.gamepad[n]->EnumObjects(DI_EnumJoypadAxesCallback,
            (void*)this, DIDFT_ABSAXIS);
    ...
    bool init(void)
    {
        ...
        DirectInput8Create(GetModuleHandle(0), 0x0800,
            IID_IDirectInput8, (void**)&device.context, 0);

        device.context->CreateDevice(GUID_SysKeyboard, &device.keyboard, 0);
        device.keyboard->SetDataFormat(&c_dfDIKeyboard);
        device.keyboard->SetCooperativeLevel(settings.handle,
            DISCL_NONEXCLUSIVE | DISCL_BACKGROUND);
        device.keyboard->Acquire();

        device.context->CreateDevice(GUID_SysMouse, &device.mouse, 0);
        device.mouse->SetDataFormat(&c_dfDIMouse2);
        HRESULT hr = device.mouse->SetCooperativeLevel(settings.handle,
            DISCL_NONEXCLUSIVE | DISCL_BACKGROUND);
        device.mouse->Acquire();

        device.context->EnumDevices(DI8DEVCLASS_GAMECTRL,
            DI_EnumJoypadsCallback, (void*)this, DIEDFL_ATTACHEDONLY);
    ...
    bool acquire(void)
    {
        ...
        if (acquired() == false) {
            device.mouse->Unacquire();
            device.mouse->SetCooperativeLevel(settings.handle,
...
BOOL CALLBACK
DI_EnumJoypadsCallback(const DIDEVICEINSTANCE *instance, void *p)
{
    return ((pInputDI*)p)->init_joypad(instance);
}

BOOL CALLBACK
DI_EnumJoypadAxesCallback(const DIDEVICEOBJECTINSTANCE *instance, void *p)
{
