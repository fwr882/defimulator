...
* SDL input driver
*
* Keyboard and mouse are controlled directly via Xlib, as SDL cannot
* capture input from windows it does not create.  SDL is only to handle
* joysticks and gamepads.
*/

#include <SDL/SDL.h>

/* FIXME: This makes it not work on Windows systems.  Not sure about Mac */
#include <sys/ipc.h>
#include <sys/shm.h>
...
struct pInputSDL {
  #include "xlibkeys.hpp"
    ...
    bool cap(const string& name)
    {
        ...
        if (name == Input::KeyboardSupport) {
            return true;
        }

        if (name == Input::MouseSupport) {
            return true;
        }

        if (name == Input::JoypadSupport) {
            return true;
        }
    ...
    bool acquire(void)
    {
        ...
        if(XGrabPointer(device.display, settings.handle, True, 0,
            GrabModeAsync, GrabModeAsync, device.rootwindow,
            device.InvisibleCursor, CurrentTime) == GrabSuccess) {

            /* backup existing cursor acceleration settings */
            XGetPointerControl(device.display, &device.accel_numerator,
                &device.accel_denominator, &device.threshold);

            /* disable cursor acceleration */
            XChangePointerControl(device.display, True, False, 1, 1, 0);

            /* center cursor (so that first relative poll returns
             * 0, 0 if mouse has not moved) */
            XWarpPointer(device.display, None, device.rootwindow, 0, 0, 0, 0,
    ...
    bool unacquire(void)
    {
        if (acquired()) {
            ...
            XChangePointerControl(device.display, True, True,
                device.accel_numerator, device.accel_denominator,
                device.threshold);
    ...
    bool poll(int16_t *table)
    {
        ...
        if (acquired()) {
            ...
            table[mouse(0).axis(0)] = (int16_t)
                (root_x_return - device.screenwidth  / 2);
            table[mouse(0).axis(1)] = (int16_t)
                (root_y_return - device.screenheight / 2);

            if (table[mouse(0).axis(0)] != 0 || table[mouse(0).axis(1)] != 0) {
                /* if movement occurred, re-center mouse for next poll */
                XWarpPointer(device.display, None, device.rootwindow,
                    0, 0, 0, 0, device.screenwidth / 2,
                    device.screenheight / 2);
            }
        } else {
            table[mouse(0).axis(0)] = (int16_t)
                (root_x_return - device.relativex);
            table[mouse(0).axis(1)] = (int16_t)
                (root_y_return - device.relativey);
        ...
        for (unsigned i = 0; i < Joypad::Count; i++) {
            ...
            /* POV hats */
            unsigned hats = min((unsigned)Joypad::Hats,
                SDL_JoystickNumHats(device.gamepad[i]));

            for (unsigned hat = 0; hat < hats; hat++) {
                uint8_t state = SDL_JoystickGetHat(device.gamepad[i], hat);
                if (state & SDL_HAT_UP) {
                    table[joypad(i).hat(hat)] |= Joypad::HatUp;
                }

                if (state & SDL_HAT_RIGHT) {
                    table[joypad(i).hat(hat)] |= Joypad::HatRight;
                }

                if (state & SDL_HAT_DOWN) {
                    table[joypad(i).hat(hat)] |= Joypad::HatDown;
                }

                if (state & SDL_HAT_LEFT) {
                    table[joypad(i).hat(hat)] |= Joypad::HatLeft;
                }
            }

            /* axes */
            unsigned axes = min((unsigned)Joypad::Axes,
                SDL_JoystickNumAxes(device.gamepad[i]));
            for (unsigned axis = 0; axis < axes; axis++) {
                table[joypad(i).axis(axis)] = 
                    (int16_t)SDL_JoystickGetAxis(device.gamepad[i], axis);
            }

            /* buttons */
            for (unsigned button = 0; button < Joypad::Buttons; button++) {
                table[joypad(i).button(button)] =
                    (bool)SDL_JoystickGetButton(device.gamepad[i], button);
            }
        }
    ...
    bool init(void)
    {
        ...
        */
        ...
        Colormap colormap = DefaultColormap(device.display,
            DefaultScreen(device.display));
        XAllocNamedColor(device.display, colormap, "black", &black, &unused);
        pixmap = XCreateBitmapFromData(device.display, settings.handle,
            invisible_data, 8, 8);
        device.InvisibleCursor = XCreatePixmapCursor(device.display, pixmap,
            pixmap, &black, &black, 0, 0);
    ...
    void term(void)
    {
        ...
        for (unsigned i = 0; i < Joypad::Count; i++) {
            if (device.gamepad[i]) {
                SDL_JoystickClose(device.gamepad[i]);
            }
            device.gamepad[i] = 0;
        }
