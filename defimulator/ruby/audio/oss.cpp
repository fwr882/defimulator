...
/*
* OSS4 soundcard.h includes below SNDCTL defines, but OSS3 does not.  However,
* OSS4 soundcard.h does not reside in <sys/>.  Therefore, attempt to manually
* define SNDCTL values if using OSS3 header.  Note that if the defines below
* fail to work on any specific platform, one can point soundcard.h above to
* the correct location for OSS4 (usually /usr/lib/oss/include/sys/soundcard.h)
* Failing that, one can disable OSS4 ioctl calls inside init() and remove the
* below defines.
*/
...
public:
    ...
    bool init(void)
    {
        ...
        */
        ...
        /* policy should be 0 - 10, lower = less latency, more CPU usage */
        int cooked = 1, policy = 4;
    ...
    pAudioOSS(void)
    {
        ...
        /* TODO: something that should be configurable at build time. */
        device.name = "/dev/dsp";
