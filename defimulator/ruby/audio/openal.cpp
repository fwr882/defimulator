...
public:
    ...
    void sample(uint16_t sl, uint16_t sr)
    {
      ...
      //wait for buffer playback to catch up to sample generation if not synchronizing
            /* wait for buffer playback to catch up to
            * sample generation if not synchronizing */
            if (settings.synchronize == false || device.queue_length < 3) {
                break;
            }
        }
        ...
        if (device.queue_length < 3) {
            ...
            alBufferData(albuffer, device.format, buffer.data,
                buffer.size * 4, settings.frequency);
