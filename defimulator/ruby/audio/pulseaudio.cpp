...
public:
    ...
    bool set(const string& name, const any& value)
    {
        ...
        if (name == Audio::Frequency) {
            ...
            if (device.stream) {
                pa_operation_unref(pa_stream_update_sample_rate(device.stream,
                    settings.frequency, NULL, NULL));
            }
            return true;
        }
        ...
        if (name == Audio::Latency) {
            ...
            if (device.stream) {
                device.buffer_attr.tlength =
                    pa_usec_to_bytes(settings.latency * PA_USEC_PER_MSEC,
                    &device.spec);
                pa_stream_set_buffer_attr(device.stream,
                    &device.buffer_attr, NULL, NULL);
            }
            return true;
        }
    }
    ...
    {
        pa_stream_begin_write(device.stream, (void**)&buffer.data,
            &buffer.size);
        buffer.data[buffer.offset++] = left + (right << 16);
        if ((buffer.offset + 1) * pa_frame_size(&device.spec) <=
            buffer.size) {
            return;
        }
        ...
        while (true) {
            ...
            if (length >= buffer.offset * pa_frame_size(&device.spec)) {
                break;
            }
        ...
        pa_stream_write(device.stream, (const void*)buffer.data,
            buffer.offset * pa_frame_size(&device.spec), NULL,
