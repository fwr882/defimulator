...
public:
    ...
    void sample(uint16_t left, uint16_t right)
    {
        ...
        if (buffer.length < device.period_size) {
            return;
        }
        ...
        do {
            ...
            if (avail < 0) {
                snd_pcm_recover(device.handle, avail, 1);
            }
        ...
        */
        ...
        if (i < 0) {
            ...
            memmove(buffer.data, buffer_ptr,
    ...
    bool init(void)
    {
        ...
        if (snd_pcm_open(&device.handle, device.name, SND_PCM_STREAM_PLAYBACK,
            SND_PCM_NONBLOCK) < 0) {
            term();
            return false;
        }
        ...
        /*
        * Below code will not work with 24khz
        * frequency rate (ALSA library bug)
        */
        #if 0
        if (snd_pcm_set_params(device.handle, device.format,
            SND_PCM_ACCESS_RW_INTERLEAVED, device.channels,
            settings.frequency, 1, settings.latency * 1000) < 0) {
            /* failed to set device parameters */
            term();
            return false;
        }

        if (snd_pcm_get_params(device.handle, &device.buffer_size,
            &device.period_size) < 0) {
            device.period_size = settings.latency * 1000 * 1e-6 *
        ...
        if (snd_pcm_hw_params_set_access(device.handle, hwparams,
            SND_PCM_ACCESS_RW_INTERLEAVED) < 0 ||
            snd_pcm_hw_params_set_format(device.handle, hwparams,
            device.format) < 0 || snd_pcm_hw_params_set_channels(
            device.handle, hwparams, device.channels) < 0 ||
            snd_pcm_hw_params_set_rate_near(device.handle, hwparams,
            &rate, 0) < 0 || snd_pcm_hw_params_set_period_time_near(
            device.handle, hwparams, &period_time, 0) < 0 ||
            snd_pcm_hw_params_set_buffer_time_near(device.handle, hwparams,
            &buffer_time, 0) < 0) {
            term();
            return false;
        }
        ...
        if (snd_pcm_sw_params_set_start_threshold(device.handle, swparams,
            (device.buffer_size / device.period_size) *
            device.period_size) < 0) {
            term();
            return false;
        }
