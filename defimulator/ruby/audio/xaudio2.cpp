...
public:
    ...
    STDMETHODIMP_(void) OnStreamEnd(void) { }
    STDMETHODIMP_(void) OnVoiceError(void *pBufferContext, HRESULT Error) { }
    STDMETHODIMP_(void) OnVoiceProcessingPassEnd(void) { }
    ...
    {
        device.buffer[device.writebuffer * device.latency +
            device.bufferoffset++] = left + (right << 16);
        if (device.bufferoffset < device.latency) {
            return;
        }
        ...
        if (device.submitbuffers == device.buffers - 1) {
            if (settings.synchronize == true) {
                /* wait until there is at least one other free
                * buffer for the next sample. */
                while (device.submitbuffers == device.buffers - 1) {
                    //Sleep(0);
                }
            } else {
                /* we need one free buffer for the next sample,
                * so ignore the current contents. */
                return;
            }
        }
    
        pushbuffer(device.latency * 4, device.buffer + device.writebuffer *
            device.latency);
    ...
    void clear(void)
    {
        ...
        /* calls OnBufferEnd for all currently submitted buffers */
        pSourceVoice->FlushSourceBuffers();
        device.writebuffer = 0;
        device.bufferoffset = 0;
        if (device.buffer) {
            memset(device.buffer, 0, device.latency * device.buffers * 4);
        }
    ...
    bool init(void)
    {
        ...
        device.buffers = 8;
        device.latency = settings.frequency *
            settings.latency / device.buffers / 1000.0 + 0.5;
        ...
        if (FAILED(hr = pXAudio2->CreateSourceVoice(&pSourceVoice,
            (WAVEFORMATEX*)&wfx, XAUDIO2_VOICE_NOSRC,
            XAUDIO2_DEFAULT_FREQ_RATIO, this, NULL, NULL))) {
            return false;
        }
