...
public:
    ...
    bool opened(void) const { return p_opened(); }
    bool open(const char *filename, mode mode_)
        { return p_open(filename, mode_); }
    void close(void) { return p_close(); }
    unsigned size(void) const { return p_size; }
    uint8_t* data(void) { return p_handle; }
    const uint8_t* data(void) const { return p_handle; }
    filemap() : p_size(0), p_handle(0) { p_ctor(); }
    filemap(const char *filename, mode mode_) : p_size(0), p_handle(0)
        { p_ctor(); p_open(filename, mode_); }
    ~filemap() { p_dtor(); }
...
private:
    ...
    bool p_open(const char *filename, mode mode_)
    {
        ...
        p_filehandle = CreateFileW(utf16_t(filename),
            desired_access, FILE_SHARE_READ, NULL,
        ...
        p_maphandle = CreateFileMapping(p_filehandle, NULL, flprotect,
            0, p_size, NULL);
        ...
        p_handle = (uint8_t*)MapViewOfFile(p_maphandle, map_access,
            0, 0, p_size);
        return p_handle;
    }
