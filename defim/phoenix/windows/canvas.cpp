    ...
    unsigned height)
{
    ...
    /* GDI stores bitmaps upside down; negative height flips bitmap */
    ...
    canvas->bmi.bmiHeader.biHeight = -height;
...
void Canvas::redraw(void)
{
    ...
    SetDIBitsToDevice(ps.hdc, 0, 0, canvas->width, canvas->height, 0, 0, 0,
        canvas->height, (void*)canvas->buffer, &canvas->bmi, DIB_RGB_COLORS);
...
static LRESULT CALLBACK Canvas_windowProc(HWND hwnd, UINT msg, WPARAM wparam,
    LPARAM lparam)
{
    ...
    switch(msg) {
    case WM_PAINT:
        object_ptr = (Object*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
        if (object_ptr) {
