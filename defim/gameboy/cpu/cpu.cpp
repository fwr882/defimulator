...
void CPU::interrupt_test(void)
{
    if (status.ime) {
        if (status.interrupt_request_vblank &&
            status.interrupt_enable_vblank) {
            status.interrupt_request_vblank = 0;
            return interrupt_exec(0x0040);
        }

        if (status.interrupt_request_stat &&
            status.interrupt_enable_stat) {
            status.interrupt_request_stat = 0;
            return interrupt_exec(0x0048);
        }

        if (status.interrupt_request_timer &&
            status.interrupt_enable_timer) {
            status.interrupt_request_timer = 0;
            return interrupt_exec(0x0050);
        }

        if (status.interrupt_request_serial &&
            status.interrupt_enable_serial) {
            status.interrupt_request_serial = 0;
            return interrupt_exec(0x0058);
        }

        if (status.interrupt_request_joypad &&
            status.interrupt_enable_joypad) {
...
void CPU::power(void)
{
    ...
    for (unsigned n = 0xc000; n <= 0xdfff; n++) {
        bus.mmio[n] = this;  //WRAM
    }

    for (unsigned n = 0xe000; n <= 0xfdff; n++) {
        bus.mmio[n] = this;  //WRAM (mirror)
    }

    for (unsigned n = 0xff00; n <= 0xff0f; n++) {
        bus.mmio[n] = this;  //MMIO
    }

    for (unsigned n = 0xff80; n <= 0xffff; n++) {
        bus.mmio[n] = this;  //HRAM+IE
    }
