    ...
    unsigned height, const string &text)
{
    widget->window = CreateWindowEx(
        ...
        WS_CHILD | WS_TABSTOP | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL |
...
{
    if (headerVisible) {
        SetWindowLong(widget->window, GWL_STYLE,
...
void ListBox::addItem(const string &text)
{
    ...
    /*
    * workaround: when there is only one column, the horizontal
    * scrollbar will always appear without this.
    */
    if (listBox->columns == 1) {
        ListView_SetColumnWidth(widget->window, 0, LVSCW_AUTOSIZE_USEHEADER);
    }
}
...
optional<unsigned> ListBox::selection(void)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        if (ListView_GetItemState(widget->window, i, LVIS_SELECTED)) {
            return { true, i };
        }
    }
...
void ListBox::setSelection(unsigned row)
{
    ...
    for(unsigned i = 0; i < count; i++) {
        if (i == row) {
            ListView_SetItemState(widget->window, i, LVIS_FOCUSED,
                LVIS_FOCUSED);
            ListView_SetItemState(widget->window, i, LVIS_SELECTED,
                LVIS_SELECTED);
        } else {
            ListView_SetItemState(widget->window, i, LVIS_FOCUSED, 0);
            ListView_SetItemState(widget->window, i, LVIS_SELECTED, 0);
        }
    }
}
