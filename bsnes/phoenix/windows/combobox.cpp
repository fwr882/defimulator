void ComboBox::create(Window &parent, unsigned x, unsigned y, unsigned width,
    unsigned height, const string &text)
{
    ...
    */
    ...
    unsigned adjustedHeight = height - ((rc.bottom - rc.top) -
        SendMessage(widget->window, CB_GETITEMHEIGHT, (WPARAM)-1, 0));
...
{
    SendMessage(widget->window, CB_ADDSTRING, 0,
        (LPARAM)(wchar_t*)utf16_t(text));
    if (SendMessage(widget->window, CB_GETCOUNT, 0, 0) == 1) {
        setSelection(0);
    }
}
