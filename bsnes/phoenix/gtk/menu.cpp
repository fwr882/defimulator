...
void Menu::create(Window &parent, const string &text)
{
    ...
    if (action->font) {
        Action_setFont(object->widget, action->font->font->font);
    }
...
void MenuItem::create(Menu &parent, const string &text)
{
    action->font = parent.action->font;
    object->widget = gtk_menu_item_new_with_label(text);
    g_signal_connect_swapped(G_OBJECT(object->widget), "activate",
        G_CALLBACK(MenuItem_tick), (gpointer)this);
...
{
    if (self->onTick && self->object->locked == false) {
        self->onTick();
    }
}
...
void MenuCheckItem::create(Menu &parent, const string &text)
{
    ...
    g_signal_connect_swapped(G_OBJECT(object->widget), "toggled",
        G_CALLBACK(MenuCheckItem_tick), (gpointer)this);
...
{
    if (self->onTick && self->checked() && self->object->locked == false) {
        self->onTick();
    }
}
...
void MenuRadioItem::create(Menu &parent, const string &text)
{
    ...
    g_signal_connect_swapped(G_OBJECT(object->widget), "toggled",
        G_CALLBACK(MenuRadioItem_tick), (gpointer)this);
...
void MenuRadioItem::create(MenuRadioItem &parent, const string &text)
{
    ...
    object->widget = gtk_radio_menu_item_new_with_label_from_widget(
        GTK_RADIO_MENU_ITEM(first->object->widget), text);
    g_signal_connect_swapped(G_OBJECT(object->widget), "toggled",
        G_CALLBACK(MenuRadioItem_tick), (gpointer)this);
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(object->parentMenu->object->menu),
        object->widget);
    gtk_widget_show(object->widget);
}
