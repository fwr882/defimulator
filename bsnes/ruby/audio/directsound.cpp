...
public:
    ...
    void sample(uint16_t left, uint16_t right)
    {
        ...
        if (settings.synchronize == true) {
            /*
            * wait until playback buffer has an empty
            * ring to write new audio data to
            */
            while (device.distance >= device.rings - 1) {
                ...
                /*
                * subtract number of played rings
                * from ring distance counter
                */
                device.distance -= (device.rings + activering -
                    device.readring) % device.rings;
                ...
                if (device.distance < 2) {
                    ...
                    device.writering = (device.rings +
                        device.readring - 1) % device.rings;
                    break;
                }
            }
        }
        ...
        if (dsb_b->Lock(device.writering * device.latency * 4,
            device.latency * 4, &output, &size, 0, 0, 0) == DS_OK) {
    ...
    void clear(void)
    {
        ...
        device.bufferoffset = 0;
        if (device.buffer) {
            memset(device.buffer, 0, device.latency * device.rings * 4);
        }
        ...
        DWORD size;
        void *output;
        dsb_b->Lock(0, device.latency * device.rings * 4,
            &output, &size, 0, 0, 0);
    ...
    bool init(void)
    {
        ...
        device.rings   = 8;
        device.latency = settings.frequency * settings.latency /
            device.rings / 1000.0 + 0.5;
