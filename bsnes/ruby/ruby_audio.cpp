...
double AudioInterface::hermite(double mu1, double a, double b, double c,
    double d)
{
...
void AudioInterface::sample(uint16_t left, uint16_t right)
{
    ...
    while (r_frac <= 1.0) {
        int output_left  = sclamp<16>(hermite(r_frac, r_left [0],
            r_left [1], r_left [2], r_left [3]));
        int output_right = sclamp<16>(hermite(r_frac, r_right[0],
            r_right[1], r_right[2], r_right[3]));
        r_frac += r_step;
        if (p) {
            p->sample(output_left, output_right);
        }
    }
...
void AudioInterface::clear(void)
{
    r_frac = 0;
