...
public:
    ...
    template<typename T> any& operator=(const T& value_) {
        typedef typename static_if<
            ...
            typename std::add_const<T>::type>::type*,T
            >::type auto_t;
    ...
    any() : container(0) {}
    template<typename T> any(const T& value_) : container(0)
        { operator=(value_); }
...
/* XXX: Apparently we're not exception-clean */
template<typename T> T any_cast(any &value)
{
    typedef typename std::remove_reference<T>::type nonref;
    if (value.type() != typeid(nonref)) {
        throw;
    }
