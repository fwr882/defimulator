...
//private:
    ...
    inline unsigned score_header(const uint8_t *data, unsigned size,
        unsigned addr);
...
SNESCartridge::SNESCartridge(const uint8_t *data, unsigned size)
{
    ...
    if (type == TypeSuperGameBoy1Bios) {
        ...
        xml << "  <spc7110>\n";
        xml << "    <mcu>\n";
        xml << "      <map address='d0-ff:0000-ffff' ";
        xml << "offset='100000' size='" << hex(size - 0x100000) << "'/>\n";
        xml << "  </mcu>\n";
    ...
    if(has_dsp1) {
        xml << "  <necdsp revision='upd7725' frequency='8000000' ";
        xml << "program='dsp1b.bin' ";
        xml << "sha256='4d42db0f36faef263d6b93f508e8c1c4ae";
        xml << "8fc2605fd35e3390ecc02905cd420c'>\n";
    ...
    if (has_dsp2) {
        xml << "  <necdsp revision='upd7725' frequency='8000000' ";
        xml << "program='dsp2.bin' sha256='5efbdf96ed065279085522";
        xml << "5964f3e90e6a4d466cfa64df25b110933c6cf94ea1'>\n";
        xml << "    <dr mask='004000' test='000000'/>\n";
        xml << "    <sr mask='004000' test='004000'/>\n";
        xml << "    <map address='20-3f:8000-ffff'/>\n";
        xml << "    <map address='a0-bf:8000-ffff'/>\n";
    ...
    if(has_dsp3) {
        xml << "  <necdsp revision='upd7725' frequency='8000000' ";
        xml << "program='dsp3.bin' sha256='2e635f72e4d4681148bc35";
        xml << "429421c9b946e4f407590e74e31b93b8987b63ba90'>\n";
        xml << "    <dr mask='004000' test='000000'/>\n";
        xml << "    <sr mask='004000' test='004000'/>\n";
        xml << "    <map address='20-3f:8000-ffff'/>\n";
        xml << "    <map address='a0-bf:8000-ffff'/>\n";
    ...
    if (has_dsp4) {
        xml << "  <necdsp revision='upd7725' frequency='8000000' ";
        xml << "program='dsp4.bin' sha256='63ede17322541c191ed1fd";
        xml << "f683872554a0a57306496afc43c59de7c01a6e764a'>\n";
        xml << "    <dr mask='004000' test='000000'/>\n";
        xml << "    <sr mask='004000' test='004000'/>\n";
    ...
    if (has_st010) {
        xml << "  <necdsp revision='upd96050' frequency='10000000' ";
        xml << "program='st0010.bin' sha256='55c697e864562445621cdf";
        xml << "8a7bf6e84ae91361e393d382a3704e9aa55559041e'>\n";
    ...
    if (has_st011) {
        xml << "  <necdsp revision='upd96050' frequency='15000000' ";
        xml << "program='st0011.bin' sha256='b71d71fa5f18ff39b2b368";
        xml << "8e914a04d1286b613002ddde827529fdc54650a130'>\n";
        xml << "    <dr mask='080001' test='000000'/>\n";
        xml << "    <sr mask='080001' test='000001'/>\n";
        xml << "    <dp mask='080000' test='080000'/>\n";
        xml << "    <map address='60-6f:0000-0fff'/>\n";
...
void SNESCartridge::read_header(const uint8_t *data, unsigned size)
{
    ...
    /* Detect Game Boy cartridges */

    if (size >= 0x0140) {
        if (data[0x0104] == 0xce && data[0x0105] == 0xed &&
            data[0x0106] == 0x66 && data[0x0107] == 0x66 &&
            data[0x0108] == 0xcc && data[0x0109] == 0x0d &&
            data[0x010a] == 0x00 && data[0x010b] == 0x0b) {
            type = TypeGameBoy;
            return;
        }
    }
    ...
    /* Detect BS-X flash cartridges */
    if (data[index + 0x13] == 0x00 || data[index + 0x13] == 0xff) {
        if (data[index + 0x14] == 0x00) {
            const uint8_t n15 = data[index + 0x15];
            if (n15 == 0x00 || n15 == 0x80 || n15 == 0x84 ||
                n15 == 0x9c || n15 == 0xbc || n15 == 0xfc) {
                if (data[index + 0x1a] == 0x33 ||
                    data[index + 0x1a] == 0xff) {
                    type = TypeBsx;
                    mapper = BSXROM;
                    /* BS-X was only released in Japan */
    ...
    /* detect Sufami Turbo cartridges */
    if (!memcmp(data, "BANDAI SFC-ADX", 14)) {
        ...
        /* Sufami Turbo only released in Japan; RAM size handled elsewhere */
        mapper = STROM;
        region = NTSC;
    ...
    /* detect standard cartridges */

    /*
    * detect presence of BS-X flash cartridge connector.
    * reads extended header information.
    */
    if (data[index - 14] == 'Z') {
        if (data[index - 11] == 'J') {
            uint8_t n13 = data[index - 13];
            if ((n13 >= 'A' && n13 <= 'Z') || (n13 >= '0' && n13 <= '9')) {
                if (company == 0x33 || (data[index - 10] == 0x00 &&
                    data[index - 4] == 0x00)) {
                    has_bsx_slot = true;
                }
            }
    ...
    if (has_bsx_slot) {
        if (!memcmp(data + index, "Satellaview BS-X     ", 21)) {
            /*
            * BS-X base cart, only released in Japan.
            * RAM size handled internally by load_cart_bsx(), which
            * is in the BSXCart class
            */
            type = TypeBsxBios;
            mapper = BSXROM;
            region = NTSC;
    ...
    if (mapperid == 0x20 && (rom_type == 0x13 || rom_type == 0x14 ||
    ...
    if (mapperid == 0x23 && (rom_type == 0x32 || rom_type == 0x34 ||
        rom_type == 0x35)) {
        has_sa1 = true;
        mapper = SA1ROM;
    }
...
unsigned SNESCartridge::find_header(const uint8_t *data, unsigned size)
{
    ...
    /* favor ExHiROM on images > 32mbits */
    if (score_ex) {
        score_ex += 4;
    }
...
unsigned SNESCartridge::score_header(const uint8_t *data, unsigned size,
    unsigned addr)
{
    ...
    uint16_t resetvector = data[addr + ResetVector] |
        (data[addr + ResetVector + 1] << 8);
    uint16_t checksum    = data[addr + Checksum   ] |
        (data[addr + Checksum    + 1] << 8);
    uint16_t complement  = data[addr + Complement ] |
        (data[addr + Complement  + 1] << 8);

    /* first opcode executed upon reset */
    uint8_t resetop = data[(addr & ~0x7fff) | (resetvector & 0x7fff)];

    /* mask off irrelevant FastROM-capable bit */
    uint8_t mapper  = data[addr + Mapper] & ~0x10;
    ...
    * have completely invalid header information that cannot be relied
    * upon.  Below code will analyze the first opcode executed at the
    * specified reset vector to determine the probability that this is
    * the correct header.
    */
    ...
    /*
    * At times, both the header and the reset vector's first opcode will
    * match... fallback and rely on info validity in these cases to determine
    * more likely header.
    *
    * A valid checksum is the biggest indicaotor of a valid header.
    */
    ...
    /* 0x33 indicates extended header */
