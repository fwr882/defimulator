...
struct ups {
    ...
    result create(const uint8_t *sourcedata, unsigned sourcelength,
        const uint8_t *targetdata, unsigned targetlength,
        const char *patchfilename)
    {
        source_data = (uint8_t*)sourcedata;
        target_data = (uint8_t*)targetdata;
        ...
        if (patch_file.open(patchfilename, file::mode::write) == false) {
            return result::patch_unwritable;
        }
        ...
        for (unsigned i = 0; i < 4; i++) {
            patch_write(source_checksum >> (i * 8));
        }

        for (unsigned i = 0; i < 4; i++) {
            patch_write(target_checksum >> (i * 8));
        }

        uint32_t patch_result_checksum = ~patch_checksum;
        for (unsigned i = 0; i < 4; i++) {
            patch_write(patch_result_checksum >> (i * 8));
        }
        ...
        uint8_t *targetdata, unsigned &targetlength)
    {
        ...
        if (source_length != source_read_length &&
            source_length != target_read_length) {
            return result::source_invalid;
        }

        if (source_length == source_read_length) {
            targetlength = target_read_length;
        } else {
            targetlength = source_read_length;
        }

        if (target_length < targetlength) {
            return result::target_too_small;
        }
        ...
        for (unsigned i = 0; i < 4; i++) {
            source_read_checksum |= patch_read() << (i * 8);
        }

        for (unsigned i = 0; i < 4; i++) {
            target_read_checksum |= patch_read() << (i * 8);
        }
        ...
        target_checksum = ~target_checksum;
        for (unsigned i = 0; i < 4; i++) {
            patch_read_checksum  |= patch_read() << (i * 8);
        }
        ...
        if (source_checksum == source_read_checksum &&
            source_length == source_read_length) {
            if (target_checksum == target_read_checksum &&
                target_length == target_read_length) {
                return result::success;
            }
            return result::target_invalid;
        } else if (source_checksum == target_read_checksum &&
            source_length == target_read_length) {
            if (target_checksum == source_read_checksum &&
                target_length == source_read_length) {
                return result::success;
            }
            return result::target_invalid;
        } else {
...
private:
    ...
    uint8_t target_read(void)
    {
        ...
        if (((target_offset++ & 255) == 0) && progress) {
            if (source_length > target_length) {
                progress(target_offset, source_length);
            } else {
                progress(target_offset, target_length);
            }
        }
        return result;
    }
