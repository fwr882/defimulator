...
class lzss {
public:
    static bool encode(uint8_t *&output, unsigned &outlength,
        const uint8_t *input, unsigned inlength)
    {
        ...
        while (i < inlength) {
            ...
            for (unsigned b = 0; b < 8 && i < inlength; b++) {
                ...
                for (unsigned index = 1; index < 4096; index++) {
                    unsigned count = 0;
                    while (true) {
                        /* verify pattern match is not
                        * longer than max length. */
                        if (count >= 15 + 3) {
                            break;
                        }

                        /* verify pattern match does
                        * not read past end of input. */
                        if (i + count >= inlength) {
                            break;
                        }
                        ...
                        * before start of input. */
                        ...
                        if (input[i + count] != input[i + count - index]) {
                            break;
                        }
                        count++;
                    }
        ...
        while (o < length) {
            ...
            for (unsigned b = 0; b < 8 && o < length; b++) {
                ...
                output[o++] = input[i++];
                    ...
                    offset &= 4095;
                    for (unsigned index = 0; index < lookuplength &&
                        o + index < length; index++) {
