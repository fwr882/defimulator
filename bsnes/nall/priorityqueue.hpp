...
/*
* Priority queue implementation using binary min-heap array; does not
* require normalize() function.
* O(1)      find    (tick)
...
public:
    ...
    {
        basecounter += ticks;
        while (heapsize && gte(basecounter, heap[0].counter)) {
            callback(dequeue());
        }
    }

    /*
    * Counter is relative to current time (eg enqeueu(64, ...) fires in 64
    * ticks.  counter cannot exceed std::numeric_limist<unsigned>::max() >> 1
    */
    ...
    type_t dequeue(void)
    {
        ...
        while (true) {
            ...
            if(child + 1 < heapsize &&
                gte(heap[child].counter, heap[child + 1].counter)) {
                child++;
            }
    ...
    priority_queue(unsigned size, function<void (type_t)>
        callback_ = &priority_queue_nocallback<type_t>)
        : callback(callback_)
    {
