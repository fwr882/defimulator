...
inline bool xml_element::parse_head(string data)
{
    ...
    for (unsigned i = 1; i < part.size(); i++) {
        ...
        if (strbegin(attr.content, "\"") && strend(attr.content, "\"")) {
            attr.content.trim<1>("\"");
        } else if(strbegin(attr.content, "'") && strend(attr.content, "'")) {
            attr.content.trim<1>("'");
        } else {
            throw "...";
        }
        ...
        while (*data) {
            ...
            if (node.parse_body(data) == false) {
                if (*data == '/') {
                    ...
                    if (length > 0) {
                        content = substr(content_begin, 0, length);
                    }
