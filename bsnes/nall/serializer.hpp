...
/*
* seralizer: a class designed to save and restore the state of classes.
* 
* benefits:
* - data() will be portable in size (it is not necessary to
*       specify type sizes.)
* - data() will be portable in endianness (always stored
*       internally as little-endian.)
* - one serialize function can both save and restore class states.
*
* caveats:
* - only plain-old-data can be stored. complex classes must provide
...
public:
    ...
    template<typename T> void floatingpoint(T &value)
    {
        ...
        /*
        * This is rather dangerous and not cross-platform safe; but
        * there is no standardized way to export floating point values
        */
    ...
    {
        enum {
            size = std::is_same<bool, T>::value ? 1 : sizeof(T)
        };

        if (imode == Save) {
            for (unsigned n = 0; n < size; n++) {
                idata[isize++] = value >> (n << 3);
            }
        } else if (imode == Load) {
            value = 0;
            for (unsigned n = 0; n < size; n++) {
                value |= idata[isize++] << (n << 3);
            }
        } else if (imode == Size) {
            isize += size;
        }
    ...
    {
        enum {
            size = sizeof(T) / sizeof(typename std::remove_extent<T>::type)
        };
