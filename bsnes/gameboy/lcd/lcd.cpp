...
void LCD::render(void)
{
    ...
    if (status.display_enable == true) {
        if (status.bg_enable == true) {
            render_bg();
        }
        if (status.window_display_enable == true) {
            render_window();
        }
        if (status.obj_enable == true) {
            render_obj();
        }
    }
...
void LCD::render_bg(void)
{
    ...
    for (unsigned ox = 0; ox < 160; ox++) {
        ...
        if (tx == 0) {
            data = read_tile(status.bg_tilemap_select, ix, iy);
        }
    }
}

void LCD::render_window(void)
{
    if (status.ly - status.wy >= 144U) {
        return;
    }
    ...
    for (unsigned ox = 0; ox < 160; ox++) {
        ...
        if (tx == 0) {
            data = read_tile(status.window_tilemap_select, ix, iy);
        }
    }
}
...
void LCD::power(void)
{
    ...
    /* VRAM */
    for (unsigned n = 0x8000; n <= 0x9fff; n++) {
        bus.mmio[n] = this;
    }

    /* MMIO */
    for (unsigned n = 0xff40; n <= 0xff4b; n++) {
        bus.mmio[n] = this;
    }

    /* OAM */
    for (unsigned n = 0xfe00; n <= 0xfe9f; n++) {
        bus.mmio[n] = this;
    }
