...
void Cartridge::MBC3::second(void)
{
    if (rtc_halt == false) {
        if (++rtc_second >= 60) {
            rtc_second = 0;
...
uint8 Cartridge::MBC3::mmio_read(uint16 addr)
{
    ...
    if ((addr & 0xe000) == 0xa000) {  //a000-bfff
        if (ram_enable) {
            ...
            if (ram_select == 0x08) {
                return rtc_latch_second;
            }

            if (ram_select == 0x09) {
                return rtc_latch_minute;
            }

            if (ram_select == 0x0a) {
                return rtc_latch_hour;
            }

            if (ram_select == 0x0b) {
                return rtc_latch_day;
            }

            if (ram_select == 0x0c) {
                return (rtc_latch_day_carry << 7) | (rtc_latch_day >> 8);
            }
        }
...
void Cartridge::MBC3::power(void)
{
    ram_enable = false;
    rom_select = 0x01;
    ram_select = 0x00;
