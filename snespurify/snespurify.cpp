...
void Application::scan(void)
{
    ...
    if (directory::exists(pathname) == false) {
        MessageWindow::warning(application,
    ...
    foreach (info, fileInfo) {
        fileList.addItem({ notdir(info.filename), "\t", info.problem,
            "\t", info.solution });
...
void Application::analyze(const string &filename)
{
    ...
    if (filename.iendswith(".sfc") || filename.iendswith(".bs")  ||
        filename.iendswith(".st")  || filename.iendswith(".gb")  ||
        filename.iendswith(".gbc") || filename.iendswith(".sgb") ||
        filename.iendswith(".smc") || filename.iendswith(".swc") ||
        filename.iendswith(".fig") || filename.iendswith(".ufo") ||
        filename.iendswith(".gd3") || filename.iendswith(".gd7") ||
        filename.iendswith(".dx2") || filename.iendswith(".mgd") ||
        filename.iendswith(".mgh") || filename.iendswith(".048") ||
        filename.iendswith(".058") || filename.iendswith(".068") ||
        filename.iendswith(".078") || filename.iendswith(".usa") ||
        filename.iendswith(".eur") || filename.iendswith(".jap") ||
        ...
        /*
        * The ordering of the rules is very important:  patches need to be
        * created prior to removal of headers.  Headers need to be removed
        * prior to renaming files (so header removal has correct filename)
        * etc.
        */
        ...
        case snes_information::TypeSuperGameBoy2Bios: {
            ...
            if (file::exists(ipsName) == true &&
                file::exists(upsName) == false) {
                FileInfo info;
        ...
        case snes_information::TypeGameBoy: {
            if (filename.endswith(".gb") == false &&
                filename.endswith(".gbc") == false &&
                filename.endswith(".sgb") == false) {
                FileInfo info;
                info.filename = filename;
                info.problem = "Wrong file extension";
...
void Application::repair(void)
{
    enable(false);
    errors.reset();

    for (unsigned n = 0; n < fileInfo.size(); n++) {
        if (fileList.checked(n) == false) {
            continue;
        }
        ...
        if (info.solution == "Create UPS patch") {
            ...
            rename(info.filename,
                string(nall::basename(info.filename), ".sfc"));
        } else if(info.solution == "Rename to .bs") {
            rename(info.filename,
                string(nall::basename(info.filename), ".bs"));
        } else if(info.solution == "Rename to .st") {
            rename(info.filename,
                string(nall::basename(info.filename), ".st"));
        } else if(info.solution == "Rename to .gb") {
            rename(info.filename,
                string(nall::basename(info.filename), ".gb"));
        }
    }

    if (errors.size() == 0) {
        MessageWindow::information(application,
            "Selected problems have been corrected");
    } else {
        string output;
        for (unsigned i = 0; i < 3 && i < errors.size(); i++) {
            output.append(string(errors[i], "\n"));
        }
...
void Application::createPatch(const string &filename)
{
    ...
    if (fp.open(filename, file::mode::read)) {
        ...
        if (psize >= 8 && pdata[0] == 'P' && pdata[1] == 'A' &&
            pdata[2] == 'T' && pdata[3] == 'C' && pdata[4] == 'H' &&
            pdata[psize - 3] == 'E' && pdata[psize - 2] == 'O' &&
            pdata[psize - 1] == 'F') {
            unsigned osize = 0;

            /* There is no way to determine how big IPS output will be; just
            * allocate max IPS patches support -- 16MB */
            ...
            ups patcher;
            if (patcher.create(widata, wisize, wodata, wosize, upsName) !=
                ups::result::success) {
                errors.append({ "Failed to create UPS patch: ",
                    upsName, "\n" });
            }
